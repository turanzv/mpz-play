...
async fn main() {
    ...
    let key = _garble_vm.new_private_input::<[u8; 16]>("key").unwrap();
    let msg = _garble_vm.new_blind_input::<[u8; 16]>("msg").unwrap();
    ...
    let mut output = _garble_vm.decode_private(&[ciphertext]).await.unwrap();

    // Print the encrypted text.
    let encrypted: [u8; 16] = output.pop().unwrap().try_into().unwrap();
    println!("ALICE\tEncrypted text is: {:?}", encrypted);
}
